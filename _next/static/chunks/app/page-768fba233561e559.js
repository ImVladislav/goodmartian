(self.webpackChunk_N_E = self.webpackChunk_N_E || []).push([
  [974],
  {
    3906: (A, t, a) => {
      Promise.resolve().then(a.bind(a, 4155));
    },
    4155: (A, t, a) => {
      "use strict";
      a.d(t, { ParallaxLanding: () => n });
      var e = a(5155),
        l = a(9417);
      let r = { src: "/_next/static/media/asteroid.4655dfc0.png" },
        s = { src: "/_next/static/media/planet.4f53c78b.png" },
        g = { src: "/_next/static/media/moon.ebe421ad.png" },
        i = { src: "/_next/static/media/rocket.8211dce6.png" };
      var c = a(5211);
      a(2115);
      var o = a(9602);
      let n = () =>
        (0, e.jsxs)(l.CH, {
          className:
            "w-full h-full absolute inset-0 flex flex-col items-center justify-center",
          globalFactorX: 0.1,
          globalFactorY: 0.1,
          children: [
            (0, e.jsx)(l.ji, {
              className:
                "absolute top-[10dvh] left-[-20%] md:left-[-20px] md:top-0 block",
              factorX: 0.2,
              factorY: 0.2,
              children: (0, e.jsx)("img", {
                src: r.src,
                alt: "Asteroid",
                className: "relative w-48 lg:w-64 xl:w-96 h-auto select-none",
              }),
            }),
            (0, e.jsx)(l.ji, {
              className:
                "absolute top-0 right-[-50px] lg:top-[5%] lg:right-[-10px] block ",
              factorX: 0.15,
              factorY: 0.15,
              children: (0, e.jsx)("img", {
                src: s.src,
                alt: "Planet",
                className: "relative  w-64 lg:w-[32rem] h-auto select-none",
              }),
            }),
            (0, e.jsx)(l.ji, {
              className:
                "absolute bottom-[75px] left-[-50px]  xs:bottom-[200px] lg:bottom-[10vh] lg:left-[10%] block",
              factorX: 0.25,
              factorY: 0.25,
              children: (0, e.jsx)("img", {
                src: g.src,
                alt: "Moon",
                className:
                  "relative w-32 xs:w-[10rem] lg:w-48 xl:w-64 2xl:w-96 h-auto select-none",
              }),
            }),
            (0, e.jsx)(l.ji, {
              className:
                "absolute right-[5px] bottom-[5px] xs:bottom-[50px] xs:right-[10px] lg:bottom-[50px] lg:right-[50px] block",
              factorX: 0.3,
              factorY: 0.3,
              children: (0, e.jsx)("img", {
                src: i.src,
                alt: "Rocket",
                className:
                  "relative w-64 xs:w-[20rem] lg:w-[28rem] 2xl:w-[34rem] 3xl:w-[50rem] h-auto select-none",
              }),
            }),
            (0, e.jsxs)(l.ji, {
              factorX: 0.1,
              factorY: 0.1,
              className: "w-10/12 lg:w-1/2 px-4 z-10",
              children: [
                (0, e.jsx)("img", {
                  src: c.default.src,
                  alt: "Martian logo",
                  className: "relative w-full h-auto select-none",
                }),
                (0, e.jsx)("div", {
                  className: "grid grid-cols-8 gap-1 max-w-lg mx-auto",
                  children: o.y.map((A) =>
                    (0, e.jsx)(
                      "a",
                      {
                        href: A.href,
                        target: "_blank",
                        children: (0, e.jsx)("img", {
                          src: A.src.src,
                          alt: "Logo",
                          className:
                            "relative object-contain select-none max-h-fit w-auto hover:scale-110 transition",
                        }),
                      },
                      A.href
                    )
                  ),
                }),
              ],
            }),
          ],
        });
    },
    9602: (A, t, a) => {
      "use strict";
      a.d(t, { cn: () => h, y: () => m });
      var e = a(3463),
        l = a(9795),
        r = a(1514),
        s = a(5224),
        g = a(8826),
        i = a(6043),
        c = a(1666),
        o = a(4314),
        n = a(3275),
        d = a(6948);
      function h() {
        for (var A = arguments.length, t = Array(A), a = 0; a < A; a++)
          t[a] = arguments[a];
        return (0, l.QP)((0, e.$)(t));
      }
      let m = [
        {
          src: s.default,
          alt: "Dex Screener logo",
          href: "https://dexscreener.com/solana/cyw9mcs6tjmtde9hgxypuxmarn3nehiwaapu4xqk4wau",
        },
        {
          src: g.default,
          alt: "Dex Tools logo",
          href: "https://www.dextools.io/app/en/token/goodmartian",
        },
        {
          src: i.default,
          alt: "Telegram logo",
          href: "https://t.me/XXXXXX",
        },

        { src: o.default, alt: "X logo", href: "https://x.com/XXXXXX" },
      ];
    },
    5211: (A, t, a) => {
      "use strict";
      a.r(t), a.d(t, { default: () => e });
      let e = {
        src: "1.gif",
        height: 589,
        width: 1328,
        blurDataURL:
          "data:image/png;base64,",
        blurWidth: 8,
        blurHeight: 4,
      };
    },
    1514: (A, t, a) => {
      "use strict";
      a.r(t), a.d(t, { default: () => e });
      let e = {
        src: "/_next/static/media/coingecko.e8ba57e8.png",
        height: 16,
        width: 151,
        blurDataURL:
          "data:image/png;base64,",
        blurWidth: 8,
        blurHeight: 8,
      };
    },
    5224: (A, t, a) => {
      "use strict";
      a.r(t), a.d(t, { default: () => e });
      let e = {
        src: "/_next/static/media/dexscreener.6c148bff.png",
        height: 156,
        width: 151,
        blurDataURL:
          "data:image/png;base64,",
        blurWidth: 8,
        blurHeight: 8,
      };
    },
    8826: (A, t, a) => {
      "use strict";
      a.r(t), a.d(t, { default: () => e });
      let e = {
        src: "/_next/static/media/dextools.7cc5a08d.png",
        height: 156,
        width: 151,
        blurDataURL:
          "data:image/png;base64,",
        blurWidth: 8,
        blurHeight: 8,
      };
    },
    1666: (A, t, a) => {
      "use strict";
      a.r(t), a.d(t, { default: () => e });
      let e = {
        src: "/_next/static/media/discord.bd48e499.png",
        height: 156,
        width: 151,
        blurDataURL:
          "data:image/png;base64,",
        blurWidth: 8,
        blurHeight: 8,
      };
    },
    3275: (A, t, a) => {
      "use strict";
      a.r(t), a.d(t, { default: () => e });
      let e = {
        src: "/_next/static/media/insta.7b6651c4.png",
        height: 156,
        width: 151,
        blurDataURL:
          "data:image/png;base64,",
        blurWidth: 8,
        blurHeight: 8,
      };
    },
    6043: (A, t, a) => {
      "use strict";
      a.r(t), a.d(t, { default: () => e });
      let e = {
        src: "/_next/static/media/telegram.bc277039.png",
        height: 156,
        width: 151,
        blurDataURL:
          "data:image/png;base64,",
        blurWidth: 8,
        blurHeight: 8,
      };
    },
    6948: (A, t, a) => {
      "use strict";
      a.r(t), a.d(t, { default: () => e });
      let e = {
        src: "/_next/static/media/tiktok.3c99c87e.png",
        height: 156,
        width: 151,
        blurDataURL:
          "data:image/png;base64,",
        blurWidth: 8,
        blurHeight: 8,
      };
    },
    4314: (A, t, a) => {
      "use strict";
      a.r(t), a.d(t, { default: () => e });
      let e = {
        src: "/_next/static/media/x.b827108e.png",
        height: 156,
        width: 151,
        blurDataURL:
          "data:image/png;base64,",
        blurWidth: 8,
        blurHeight: 8,
      };
    },
  },
  (A) => {
    var t = (t) => A((A.s = t));
    A.O(0, [181, 417, 441, 517, 358], () => t(3906)), (_N_E = A.O());
  },
]);
